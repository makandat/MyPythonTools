#!/usr/bin/env python3
#  ディレクトリ内のファイルサイズの合計と最新及び最古のファイルの日付を得る。(サブディレクトリを含む)
...
# サブディレクトリ一覧を取得
dirlist = listDirRecursively(path)
# ファイルサイズの合計、最新と最古のファイルを表示する。
for dir in dirlist :
  listFiles(dir)

# 対象ディレクトリ内のサブディレクトリの一覧を再帰的に得る。
def listDirRecursively(path) :
  return

#
#  ディレクトリ内のファイルサイズの合計、最新と最古のファイルを表示する。
#  ==============================================================
def listFiles(path) :
  # 対象ディレクトリ内のファイル一覧を得る。
